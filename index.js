...
module.exports = {
  ...
  deriveKey: function(password, salt, options, cb) {
    ...
    if (options.kdf === 'scrypt') {
      if (isFunction(scrypt)) {
        scrypt = scrypt(
          options.kdfparams.memory || self.constants.scrypt.memory
        )
      }
      if (isFunction(cb)) {
        setTimeout(function() {
   ...
   */
  exportToFile: async function(keyObject, keystore, datadir, cb) {
    var self = this
    ...
    // Create keystore folder if not present already

    var keyStoreFolderPath = path.join(datadir, keystore)

    var isKeyStorePresent = await fs.exists(keyStoreFolderPath)
    __DEV__ && console.log('mkdirPath', isKeyStorePresent, keyStoreFolderPath)

    if (!isKeyStorePresent) {
      await fs.mkdir(keyStoreFolderPath)
    }

    outpath = path.join(keyStoreFolderPath, outfile)
    json = JSON.stringify(keyObject)
    console.log('exportToFile', outpath)
    // if (this.browser) {
    ...
    fs
      ...
      .then(success => {
        __DEV__ && console.log('FILE WRITTEN!')
        instructions(outpath)
        cb && cb(outpath, keyObject)
      })
      .catch(err => {
        __DEV__ && console.log('Error occurred!', err)
      })
  },
    ...
    fs
      ...
      .then(files => {
        ...
        fs
          ...
          .catch(e => {
            __DEV__ && console.log('An error occurred - importFromFile', e)
            if (e) return cb(e)
          })
      })
      .catch(e => {
